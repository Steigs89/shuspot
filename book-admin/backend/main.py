...
@app.post("/shuspot-ingestion/upload-zip-to-supabase")
async def upload_zip_to_supabase(
    zip_file: UploadFile = File(...),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail="rclone not found. Please install rclone and configure supabase-remote")
            ...
            for book_data in books:
                ...
                if book_data.get('_page_sequence'):
                    for pg in book_data['_page_sequence']:
                        if isinstance(pg, dict) and pg.get('file_path'):
                            pg['file_path'] = convert_to_supabase_url(pg['file_path'])
            ...
            return {
                "message": f"ZIP uploaded to Supabase and {imported_count} books created/updated in database",
                "uploaded_files": True,
...
@app.post("/api/shuspot-ingestion/upload-zip-to-supabase")
async def api_upload_zip_to_supabase(zip_file: UploadFile = File(...), db: Session = Depends(get_db)):
    return await upload_zip_to_supabase(zip_file=zip_file, db=db)
