-- Supabase Database Schema for Read to Me App
...
CREATE TABLE books (
    ...
    content_url TEXT, -- Alibaba Cloud OSS URL
...
BEGIN
    ...
    SELECT MAX(DATE(start_time)) INTO last_read
    FROM reading_sessions
    WHERE user_id = user_uuid AND duration_seconds > 300; -- At least 5 minutes

    -- Calculate current streak
    WITH RECURSIVE streak_calc AS (
        SELECT DATE(start_time) as read_date, 1 as streak
        FROM reading_sessions
        WHERE user_id = user_uuid 
        AND DATE(start_time) = last_read
        AND duration_seconds > 300
        
        UNION ALL
        
        SELECT rs.read_date, sc.streak + 1
        FROM (
            SELECT DISTINCT DATE(start_time) as read_date
            FROM reading_sessions
            WHERE user_id = user_uuid AND duration_seconds > 300
        ) rs
        JOIN streak_calc sc ON rs.read_date = sc.read_date - INTERVAL '1 day'
    )
    SELECT MAX(streak) INTO current_streak FROM streak_calc;

    -- Update or insert streak record
    INSERT INTO reading_streaks (user_id, current_streak, longest_streak, last_read_date, streak_start_date)
    VALUES (user_uuid, COALESCE(current_streak, 0), GREATEST(COALESCE(current_streak, 0), longest_streak), last_read, last_read)
    ON CONFLICT (user_id) DO UPDATE SET
        current_streak = COALESCE(current_streak, 0),
        longest_streak = GREATEST(reading_streaks.longest_streak, COALESCE(current_streak, 0)),
